...
n_trees <- as.integer(200)  # IF Bäume

###############################################################################
# 3) librosa-like helpers (STFT + Mel + dB)
###############################################################################

hann_window <- function(n) {
  k <- 0:(n - 1)
  0.5 - 0.5 * cos(2 * pi * k / n)  # periodic Hann (librosa default)
}
...
stft_librosa_like <- function(y, n_fft, hop_length, center = TRUE) {
  ...
  if (ncol(frames) == 0) {
    return(matrix(complex(real = numeric(0), imaginary = numeric(0)),
                  nrow = n_freqs, ncol = 0))
  }
...
# Slaney mel conversions (librosa default)
...
mel_filterbank_librosa_like <- function(sr, n_fft, n_mels, fmin = 0, fmax = sr/2) {
  ...
  # Slaney normalization (norm="slaney" in librosa)
...
###############################################################################
# 4) Precompute Mel Filterbank
###############################################################################
fb_global <- mel_filterbank_librosa_like(sample_rate, n_fft, n_mels)

###############################################################################
# 5) compute_logmel_context (R-only, librosa-like)  <-- analog ersetzt
###############################################################################
...
###############################################################################
# 7/8) PCA + IF mit 100 Bootstrap-Runs (insgesamt) + AUCs speichern
#      -> Dieser Block ersetzt den bisherigen PCA+IF-Loop + Überblick komplett.
###############################################################################
...
# --- Helper: PCA+IF train+eval for one ID with bootstrapped train clips ---
run_one_id_pca_if <- function(id, boot_train_files_for_id,
                              max_frames_per_clip = NULL) {
  
  # Training features (bootstrapped list of matrices)
  ...
  if (is.null(X_train) || nrow(X_train) < 5) return(NULL)
  ...
  # Clip labels (normal/anomaly) from filename
  y_test_clip <- ifelse(
  ...
  ######################################################################
  # 1) PCA fit on training frames (DCASE-konform: Training ist normal)
  ######################################################################
  pca_model <- prcomp(X_train, center = TRUE, scale. = TRUE)
  
  # choose k to reach variance threshold
  var_explained <- cumsum(pca_model$sdev^2) / sum(pca_model$sdev^2)
  k <- which(var_explained >= pca_var_explained)[1]
  if (is.na(k) || k < 1) k <- 1L
  
  # project
  Z_train <- pca_model$x[, 1:k, drop = FALSE]
  Z_test  <- scale(X_test,
                   center = pca_model$center,
                   scale  = pca_model$scale) %*% pca_model$rotation[, 1:k, drop = FALSE]
  
  ######################################################################
  # 2) Isolation Forest on PCA space
  ######################################################################
  ss <- min(256L, nrow(Z_train))
  ...
  ######################################################################
  # 3) Scores + Clip aggregation
  ######################################################################
  frame_scores <- as.numeric(predict(if_model, Z_test))
...
for (r in seq_len(R)) {
  for (id in unique_ids) {
    ...
    # Speed option: set e.g. 200L, or leave NULL for full frames
    out <- run_one_id_pca_if(id, boot_train_files_for_id, max_frames_per_clip = NULL)
...
saveRDS(results_df, file = "bootstrap_pca_if_results.rds")
write.csv(results_df, file = "bootstrap_pca_if_results.csv", row.names = FALSE)
