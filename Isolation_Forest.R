...
###############################################################################
# 9) Helper: train+eval for one ID given bootstrapped train+test file list
###############################################################################
run_one_id_if <- function(id,
  ...
  # Clip labels from filename (order matches boot_test_files_for_id)
  y_test_clip <- ifelse(
...
for (r in seq_len(R)) {
  for (id in unique_ids) {
    ...
    if (length(test_idx) == 0) { k <- k + 1L; setTxtProgressBar(pb, k); next }
