...
###############################################################################
# 5) compute_logmel_context (R-only, librosa-like)
###############################################################################
...
# --- Helper: PCA+IF train+eval for one ID with bootstrapped train+test clips ---
run_one_id_pca_if <- function(id,
  ...
  # Clip labels (normal/anomaly) from filename (order matches boot_test_files_for_id)
  y_test_clip <- ifelse(
